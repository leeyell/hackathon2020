...
with torch.no_grad():
    for i, (video_id, sample) in enumerate(data_loader):
        ...
        for j in range(0, sample.size(2) - 16, 8):
            output = softmax(net(sample[:, :, j:j + 16]))
            #outputs.append(output)
            
            output_max = output.max(dim=1)
            curr_class = output_max.indices.item()
            
            if output_max.values.item() > threshold and curr_class != prev_class:
                timestamps.append(f'{str(j // 8 // 60).zfill(2)}:{str(j // 8 % 60).zfill(2)}')
                ts_classes.append(curr_class)
                prev_class = curr_class

        row_data = {'time_stamp':timestamps, 'time_stamp_class':ts_classes}
        print(f'{i + 1}/{len(data_loader)}: {row_data}')
        data[video_id.item()] = row_data
...
for ax, img in zip(axes, samples.permute(0, 2, 3, 1)):
    ...
    ax.axis('off')
plt.show()
plt.imshow(torch.repeat_interleave(outputs.T, 2, dim=0).cpu())
plt.axis('off')
plt.show()'''