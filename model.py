...
class SlowFast(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], class_num=10, dropout=0.5 ):
        ...
        self.fast_res2 = self._make_layer_fast(block, 8, layers[0], head_conv=3)    # 보틀넥, 커널 수, 연속 레이어 수
    ...
    # Resnet 에서 conv layer 연속으로 나오는 구간 정의
    def _make_layer_fast(self, block, planes, blocks, stride=1, head_conv=1):
        downsample = None
        if stride != 1 or self.fast_inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                # nn.Conv3d(in_channel, out_chennel, kernel_size, stride, ...)
                nn.Conv3d(
                    self.fast_inplanes,
        ...
        self.fast_inplanes = planes * block.expansion   # fast_inplanes 는 이 함수를 거칠 때마다 증가함
